...
$local_deals_base = 'http://www.travelzoo.com/local-deals/deal/';

//Need to check each url every run
$flag=0; //parametro que terminara el ciclo
$x = 1; // inciamos la variable que nos representara los deals, siemore debe ser 1
$datadate = date('Y-m-d');
...
while( $x < 10000000000) {  //aqui ponemos el limite superior hasta donde queremos que el script se ejecute
  ...
  if($http_response_header[0]!="HTTP/1.0 200 OK")
		...
		$flag++;
		echo $flag;
		if ($flag>50)// aqui verifco si hay mas de 50m registros vacion le digo al script que termine
		{
			break;
		}
		continue;
